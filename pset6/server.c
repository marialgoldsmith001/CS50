...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // Get line without CRLFs
            char tokens[needle - haystack +1];
            strncpy(tokens, haystack, needle - haystack);
            tokens[needle-haystack] = '\0';
            
            // Extract (method, request-target, http-version) without sp
            char* request[3] = {0, 0, 0};
            request[METHOD]= strtok(tokens, " ");
            int i = 0;
            while (request[i] != NULL && i < 2)
            {
                i++;
                request[i] = strtok(NULL, " ");
            }
            // check for method not null
            if(request[METHOD] == NULL || strcmp("GET", request[METHOD]) != 0)
            {
                //error: Method not allowed
                error(405);
                continue;
            }
            // Check for request-target not null
            
            // 1st if: make sure it begins with "/"
            // 2nd if: make sure that request-target has no "
            if(request[REQUEST_TARGET] == NULL)
            {
                //error: Bad Request
                error(400);
                continue;
            }
            else if (request[REQUEST_TARGET][0] != '/')
            {
                //error: Not Implemented
                error(501);
                continue;
            }
            else if (strchr(request[REQUEST_TARGET], '\"') != NULL)
            {
                //error: bad request
                error(400);
                continue;
            }
            // Check HTTP-VERSION no null
            if(request[HTTP_VERSION] == NULL || strcmp("HTTP/1.1", request[HTTP_VERSION]) !=0)
            {
            ...
            char* queryq = strchrnul(request[REQUEST_TARGET], '?');
            char* eosq = strchr(request[REQUEST_TARGET], '\0');
            char query[(eosq - queryq < 1) ? 1 : eosq - queryq];
            memcpy(query, (strncmp(queryq, "\0", 1) == 0)? queryq : queryq + 1, (eosq - queryq < 1) ? 1 : eosq - queryq);
            
            // Get pathway
            queryq = strchrnul(request[REQUEST_TARGET], '?');
            char pathway[queryq - request[REQUEST_TARGET] +1];
            strncpy(pathway, request[REQUEST_TARGET], queryq - request[REQUEST_TARGET]);
            pathway[queryq - request[REQUEST_TARGET]] = '\0';
            
            //Check for path missing "."
            char* extens= strchr(pathway, '.');
            if(extens == NULL)
            {
                // error: Not Implemented
                error(501);
            ...
            // extract extension
            eosq = strchr(pathway, '\0');
            char extension[eosq - extens];
            memcpy(extension, extens + 1, eosq - extens);
            
            // concatenate root and absolute-path
            char path[strlen(root) + strlen(pathway) +1];
            strcpy(path, root);
            strcat(path, pathway);
